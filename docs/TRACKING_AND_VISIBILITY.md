...
Image visibility estimates the energy remaining in the source area. It can
dim and shrink artistic elements as branches cross the source. Lens Lab's physical
ghosts dim without shrinking their geometry. It needs a clear view of
...
Cover the whole luminous body, not just its hottest pixel: at 1080p a radius of
0.02 is 21.6 pixels. The surrounding ring at two to three times the radius
estimates background. Subtraction is conservatively bounded when that ring is
as bright as the source, but this cannot identify an emitter's correct size.
