...
export function openPresetGallery({index, selected, previewUrl, onChoose, opener}) {
  ...
  heading.append(infoTag('Previewing is safe: it does not edit your flare. Load replaces the current group’s look; Merge adds its artistic elements. Lens-inspired presets are independent interpretations, not measured replicas of commercial lenses.','About preset library'));
  ...
  const subcategory=document.createElement('select');subcategory.className='fcore-preset-category';subcategory.setAttribute('aria-label','Preset subcategory');
  const filters=document.createElement('div');filters.className='fcore-preset-filters';filters.append(category,subcategory);
  const fillSubcategories=()=>{
    const previous=subcategory.value;subcategory.replaceChildren();
    const candidates=index.filter(p=>!category.value || (p.category || 'Other')===category.value);
    for(const value of ['',...new Set(candidates.map(p=>p.subcategory || 'Uncategorized'))].sort((a,b)=>a.localeCompare(b))){
      const option=document.createElement('option');option.value=value;option.textContent=value || 'All subcategories';subcategory.append(option);
    }
    if([...subcategory.options].some(o=>o.value===previous))subcategory.value=previous;
  };
  fillSubcategories();
  ...
  dialog.append(header,search,filters,body);document.body.append(dialog);
  ...
  const show=(item)=>{
    ...
    if(item.subcategory?.startsWith('Lens inspired'))info.append(' · Independent artistic interpretation; not a measured commercial lens replica.');
    if(item.subcategory?.startsWith('Reference studies'))info.append(' · Procedural study of the labelled focal length and T-stop. Not a calibrated replica or continuous zoom model. Load activates the artistic stack; Lens Lab tracing is not used.');
  ...
  const draw=()=>{
    ...
    const items=index.filter(p=>(!category.value || (p.category || 'Other')===category.value) && (!subcategory.value || (p.subcategory || 'Uncategorized')===subcategory.value) && [p.title,p.name,p.category,p.subcategory].join(' ').toLowerCase().includes(query));
  ...
  search.oninput=draw;
  category.onchange=()=>{fillSubcategories();draw();};
  subcategory.onchange=draw;
