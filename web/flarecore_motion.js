...
export function createMotionPanel(element, onChange, schemaLoader=loadSchema, uiState={}) {
  ...
  schemaLoader().then(schema=>{
    ...
    label.append(infoTag('Enable this element’s saved response curves. They change artistic element properties as the light moves; they do not modify Lens Lab ray tracing. Turning off preserves the curves.','About optical response'));
    ...
    function select(parent,labelText,value,choices,onchange) {
      const lab=el('label',labelText+' ',parent);
      const help={Property:'The element property changed by this curve. Switching property resets each point to the new property’s neutral value.',
        'Driven by':'The light measurement used as the horizontal curve input. Switching driver redistributes point positions over the new driver’s range. Read the driver description below for its units.',
        Curve:'Smooth eases between points; Linear uses straight ramps. Values outside the authored range hold the nearest endpoint. This is a response to source position, not a timeline.'};
      if(help[labelText])lab.append(infoTag(help[labelText],`About ${labelText}`));
      const s=el('select',undefined,lab);
