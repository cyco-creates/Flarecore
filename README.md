...
## Experimental Lens Lab

The **Lens Lab** button in Flarecore Render opens a physical lens designer
inside ComfyUI. Its first reference lens traces spherical refractions and
two-reflection ghosts, with a selectable optical diagram, aperture controls,
sensor controls, spectral single-layer coating studies and per-surface
reflection suppression. A separate artistic source glow/ray finish can be
disabled to inspect only traced ghosts. Hover or focus the small **i** tags
beside settings for explanations and units. Existing
presets remain unchanged until you enable it; artistic elements can be retained
as an optional hybrid layer. This is an experimental constant-index prototype,
not measured commercial lens data, physical diffraction, anamorphic optics or zoom.

See [Lens Lab usage, limitations and architecture](docs/LENS_LAB.md).
...
coating variants such as blue, amber, silver and clear.

The **Lens inspired** shelves add 15 independent artistic studies of commercial
cinema lens families: five modern spherical, four character spherical and six
anamorphic looks. Use category/subcategory filters or search in the preset
gallery. These are editable interpretations, not measured lens replicas or new
physical prescriptions. See [the collection and research notes](docs/LENS_INSPIRED_PRESETS.md).

Element families are `glows`, `ghosts`, `rays`, `streaks`, `rings`, `hoops`, `caustics`, and
`lens_dirt`.
