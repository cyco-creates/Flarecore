...
def apply_source_visibility(linear_chunk, lights_per_frame, chunk, radius,
    ...
    for entries in groups.values():
        ...
        # A bright annulus can contain the emitter itself (an extended lamp,
        # not dark background). Subtracting almost the entire reference then
        # turns a small flux loss into complete extinction. Bound subtraction
        # by the measured source-to-surround contrast: the denominator stays
        # at least half the reference, with a continuous conservative fallback
        # to relative aperture flux when source and surroundings are alike.
        # This cannot infer emitter size; the aperture must cover its body.
        pedestal = min(pedestal, max(ref-pedestal, 0.))
