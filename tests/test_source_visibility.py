...
def test_bright_annulus_cannot_extinguish_a_mostly_visible_emitter():
    # A small sampling aperture inside an extended luminous body. Its annulus
    # is not a valid dark-background estimate, even though it is below peak.
    clip=torch.full((5,64,96,3), .92)
    yy,xx=torch.meshgrid(torch.arange(64),torch.arange(96),indexing='ij')
    disc=(xx-48)**2+(yy-32)**2 <= 10**2
    for i,value in enumerate([1., .87, .01, .87, 1.]):
        clip[i,disc]=value
    v=[f[0]['source_visibility'] for f in measure(clip,radius=.06)]
    assert v[0]>.99 and .8<v[1]<.9 and v[2]==0
    assert v[1]==pytest.approx(v[3]) and v[4]>.99


def test_background_fallback_changes_continuously():
    clip=covered_clip()
    results=[]
    for background in [.49,.5,.51]:
        sample=clip.clone()
        sample[sample==.15]=background
        results.append(measure(sample)[1][0]['source_visibility'])
    assert max(results)-min(results)<.04
