...
def test_catalog_distinguishes_artistic_physical_and_hybrid(tmp_path,monkeypatch):
    for name,config in [('artistic',{}),('physical',{'enabled':True}),('hybrid',{'enabled':True,'include_artistic':True})]:
        (tmp_path/(name+'.json')).write_text(preset_json({'name':name,'elements':[],'lens_lab':config}),encoding='utf-8')
    monkeypatch.setattr(PKG.nodes.api,'PRESETS_DIR',tmp_path)
    assert {row['name']:row['render_mode'] for row in PKG.nodes.api._preset_index()}=={
        'artistic':'artistic','physical':'physical','hybrid':'hybrid'}
