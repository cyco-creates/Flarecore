...
def _preset_index() -> list[dict]:
    ...
    for path in sorted(PRESETS_DIR.glob("*.json")):
        entry = {"name": path.stem, "title": "", "category": "",
                 "subcategory": "", "render_mode": "artistic"}
        try:
            data = json.loads(path.read_text(encoding="utf-8"))
            entry["title"] = str(data.get("name", ""))
            entry["category"] = str(data.get("category", ""))
            entry["subcategory"] = str(data.get("subcategory", ""))
            lens = data.get("lens_lab") or {}
            if lens.get("enabled"):
                entry["render_mode"] = "hybrid" if lens.get("include_artistic") else "physical"
...
def _render_preset_png(text: str, width: int = 512, height: int = 288) -> bytes:
    ...
    if not optical.get("enabled") or optical.get("include_artistic"):
...
def register_routes() -> bool:
    ...
    from .lens_lab_api import register_lens_routes
